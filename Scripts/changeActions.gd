...
var actionChanger = preload("res://Scenes/actionChanger.tscn")
...
func _ready():
	...
	Singleton.sortAllActions()
	for action in Singleton.activeActionList:
		var actnBtn = actionChanger.instance()
		actnBtn.get_node("Label").text = action
		$ScrollContainer/VBoxContainer.add_child(actnBtn)
		actnBtn.rect_scale.x = 0.1
		actnBtn.rect_scale.y = 0.1
		pass
	for action in Singleton.disabledActionList:
		var actnBtn = actionChanger.instance()
...
func _on_AddAction_pressed():
	...
	if($LineEdit.text == ""):
	...
	elif(($LineEdit.text).to_lower() in Singleton.activeActionList):
		$errorTimer.start(3)
		$ERROR.visible = true
		$empty.visible = false
		...
		Singleton.activeActionList.append($LineEdit.text)
		var actnBtn = actionChanger.instance()
